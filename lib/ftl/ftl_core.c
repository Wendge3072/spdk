...
ftl_needs_reloc(struct spdk_ftl_dev *dev, bool *background_gc)
{
	...
	bool comp_idle = ftl_get_bggc_threshold(dev) > comp_bw;
	bool user_idle = ftl_get_bggc_threshold(dev) > user_bw;
...
void ftl_print_per_sec(struct spdk_ftl_dev *dev){
	...
	if(tsc - dev->last_print_tsc > spdk_get_ticks_hz()){
		...
		// uint64_t valid_blocks_in_bands = 0;
		// struct ftl_band *band;
		// TAILQ_FOREACH(band, &dev->shut_bands, queue_entry){
		// 	valid_blocks_in_bands += band->p2l_map.num_valid;
		// }
		...
		FTL_NOTICELOG(dev, "Max Invalidity: %.2f %%\n", dev->reloc->Max_invalidity * 100.0);
...
spdk_ftl_set_switch(struct spdk_ftl_dev *dev, uint32_t switch_number){
	assert(dev);
	dev->conf.switches ^= (1L << switch_number);
	if (dev->conf.switches & (1L << switch_number)) {
